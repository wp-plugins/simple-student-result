...
register_activation_hook(SSR_ROOT_FILE,'ssr_plugin_install');function ssr_plugin_install(){global $wpdb;$table_name=$wpdb->prefix.SSR_TABLE;if($wpdb->get_var("SHOW TABLES LIKE '$table_name'")!=$table_name){global $jal_db_version;$charset_collate='';if(!empty($wpdb->charset)){$charset_collate="DEFAULT CHARACTER SET {$wpdb->charset}";}if(!empty($wpdb->collate)){$charset_collate.=" COLLATE {$wpdb->collate}";}$sql="CREATE TABLE ".$wpdb->prefix.'ssr_studentinfo'." (
		...
		update_option('ssr_search_box_text','Enter Registration ID');
		update_option('ssr_search_box_no_result_text','No Results !');
		update_option('ssr_search_box_ssr_slug','Student');
		update_option('ssr_search_box_ssr_item1','Registration Number');		
		update_option('ssr_search_box_ssr_item2','Roll No');
		update_option('ssr_search_box_ssr_item3','Student Name');
		update_option('ssr_search_box_ssr_item4','Fathers Name');
		update_option('ssr_search_box_ssr_item5','Passing Year');
		update_option('ssr_search_box_ssr_item6','CGPA');
		update_option('ssr_search_box_ssr_item7','Subject');
		update_option('ssr_menu_slug','Students Result');
		update_option('ssr_menu_slug_add','Add Student Results');
		update_option('ssr_menu_1st_custom_slug','CGPA');
		update_option('ssr_menu_2nd_custom_slug','Subject');
		update_option('ssr_search_box_heading','Online Result System');
		}
